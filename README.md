...
| **Всего** | **41** | **Число объявленных тестов, без отдельного подсчёта `subTest`** | |
...
Существующий JS-набор marketplace даёт **11/11 ошибок и в текущей копии, и на исходном HEAD до доработки**: тестовая имитация браузера не предоставляет `window.dispatchEvent`, вследствие чего нарушаются проверки запросов и отображения. Это прежняя проблема тестового окружения; файлы marketplace в этой доработке не менялись. Серверные Python-тесты в рамках изменения интерфейса рейтинга повторно не запускались.
